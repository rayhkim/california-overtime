...
set_earnings = function(df) {
  ...
  df$EARNINGS = 0
  df$EARNINGS = df$HOURWAGE * df$HOURS
  df[df$HOURS > df$STANDARDHOURS,]$EARNINGS = df[df$HOURS > df$STANDARDHOURS,]$EARNINGS +
    0.5 * df[df$HOURS > df$STANDARDHOURS,]$HOURWAGE *
    (df[df$HOURS > df$STANDARDHOURS,]$HOURS - df[df$HOURS > df$STANDARDHOURS,]$STANDARDHOURS)
  return(df)
}

set_hours = function(df) {
  df = filter(df, (UHRSWORK1 <= 168) | (AHRSWORK1 <= 168),
              UHRSWORK1 > 0, AHRSWORK1 > 0)
  df$HOURS = ifelse(df$UHRSWORK1 <= 168, df$UHRSWORK1, df$AHRSWORK1)
  return(df)
}
...
get_did_data = function(data, start_year, end_year, dep_var, clusters) {
  df = filter(data, IND > 0,
              OCC > 0,
              YEAR >= start_year,
              YEAR <= end_year)
